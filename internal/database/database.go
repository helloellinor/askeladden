...
func New(cfg *config.Config) (*DB, error) {
	...
	if err != nil {
		logger.Error("Could not open database connection: %v", err)
		return nil, err
	}

	if err := conn.Ping(); err != nil {
		logger.Error("Could not test database connection: %v", err)
		return nil, err
	}
	...
	if cfg.TableSuffix != "" {
		...
		logger.Debug("Using beta table names: %s, %s, %s", tableName, bannedWordsTable, starboardTable)
	}
	...
	if err := db.createTables(); err != nil {
		logger.Error("Could not create tables: %v", err)
		return nil, err
	}
	...
	logger.Debug("Creating table: %s", db.bannedWordsTable)
...
func (db *DB) createTables() error {
	...
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Creating table if not exists: %s", db.tableName)
	...
	logger.Debug("Creating table if not exists: %s", db.starboardTable)
...
func (db *DB) resetTables() error {
	...
	logger.Debug("Creating table: %s", db.tableName)
	if _, err := db.conn.Exec(questionsQuery); err != nil {
		return fmt.Errorf("failed to create %s table: %w", db.tableName, err)
	}
...
func (db *DB) runMigrations() error {
	...
	if err != nil {
		logger.Debug("Failed to check if forum_thread_id column exists: %v", err)
		return err
	}

	if columnExists == 0 {
		logger.Debug("Adding forum_thread_id column to %s table", db.bannedWordsTable)
		...
		if err != nil {
			logger.Debug("Failed to add forum_thread_id column: %v", err)
			return err
		}
		logger.Debug("Successfully added forum_thread_id column to %s", db.bannedWordsTable)
	} else {
		logger.Debug("forum_thread_id column already exists in %s", db.bannedWordsTable)
	}
...
func (db *DB) AddQuestion(question, authorID, authorName, messageID, channelID string) (int64, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Info("Adding question from user %s: %s", authorName, question)
	...
	if err != nil {
		logger.Error("Failed to add question: %v", err)
		return 0, err
	}
	...
	logger.Debug("Successfully added question with ID %d", id)
	return id, nil
}
...
func (db *DB) ApproveQuestion(questionID int, approverID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Approving question ID %d by approver %s", questionID, approverID)
	...
	if err != nil {
		logger.Debug("Failed to approve question ID %d: %v", questionID, err)
		return err
	}
	logger.Debug("Successfully approved question ID %d", questionID)
	return nil
}
...
func (db *DB) RejectQuestion(questionID int, rejectorID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Rejecting question ID %d by rejector %s", questionID, rejectorID)
	...
	if err != nil {
		logger.Debug("Failed to reject question ID %d: %v", questionID, err)
		return err
	}
	logger.Debug("Successfully rejected question ID %d", questionID)
	return nil
}
...
func (db *DB) GetPendingQuestion() (*Question, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Retrieving next pending question")
	...
	if err != nil {
		...
		logger.Debug("Failed to get pending question: %v", err)
		return nil, err
	}
	logger.Debug("Retrieved pending question ID %d: %s", q.ID, q.Question)
	return &q, nil
}
...
func (db *DB) UpdateApprovalMessageID(questionID int, approvalMessageID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Updating approval message ID for question %d: %s", questionID, approvalMessageID)
	...
	if err != nil {
		logger.Debug("Failed to update approval message ID for question %d: %v", questionID, err)
		return err
	}
	logger.Debug("Successfully updated approval message ID for question %d", questionID)
	return nil
}
...
func (db *DB) GetQuestionByApprovalMessageID(approvalMessageID string) (*Question, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Looking up question by approval message ID: %s", approvalMessageID)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Debug("No question found for approval message ID: %s", approvalMessageID)
		} else {
			logger.Debug("Failed to get question by approval message ID %s: %v", approvalMessageID, err)
		}
		return nil, err
	}
	logger.Debug("Found question ID %d for approval message %s", q.ID, approvalMessageID)
	return &q, nil
}
...
func (db *DB) GetPendingQuestionByID(questionID int) (*Question, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Looking up pending question by question ID: %d", questionID)
	...
	logger.Debug("[DEBUG] SQL Query: %s", query)
	var q Question
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Debug("No pending question found with ID: %d", questionID)
		} else {
			logger.Debug("Failed to get pending question by ID %d: %v", questionID, err)
		}
		return nil, err
	}
	logger.Debug("[DATABASE] Found pending question ID %d", q.ID)
	return &q, nil
}
...
func (db *DB) GetLeastAskedApprovedQuestion() (*Question, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Retrieving least asked approved question")
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Debug("[DATABASE] No approved questions found")
			return nil, nil
		} else {
			logger.Debug("[DATABASE] Failed to get least asked approved question: %v", err)
			return nil, err
		}
	}
	logger.Debug("[DATABASE] Retrieved least asked approved question (asked %d times): %s", q.TimesAsked, q.Question)
	return &q, nil
}
...
func (db *DB) IncrementQuestionUsage(questionID int) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("[DATABASE] Incrementing usage count for question ID %d", questionID)
	...
	if err != nil {
		logger.Debug("[DATABASE] Failed to increment usage count for question ID %d: %v", questionID, err)
		return err
	}
	logger.Debug("[DATABASE] Successfully incremented usage count for question ID %d", questionID)
	return nil
}
...
func (db *DB) AddBannedWord(word, reason, authorID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Adding banned word: %s by %s", word, authorID)
	...
	if err != nil {
		logger.Debug("Failed to add banned word: %v", err)
		return err
	}
	logger.Debug("Successfully added banned word: %s", word)
	return nil
}
...
func (db *DB) AddBannedWordPending(word, reason, authorID, authorName, forumThreadID, originalMessageID string) (int64, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Adding pending banned word: %s by %s", word, authorID)
	...
	if err != nil {
		logger.Debug("Failed to add pending banned word: %v", err)
		return 0, err
	}
	id, err := result.LastInsertId()
	if err != nil {
		logger.Debug("Failed to get last insert ID for pending banned word: %v", err)
		return 0, err
	}
	logger.Debug("Successfully added pending banned word with ID %d", id)
	return id, nil
}
...
func (db *DB) UpdateBannedWordApprovalMessageID(wordID int, approvalMessageID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Updating approval message ID for banned word %d: %s", wordID, approvalMessageID)
	...
	if err != nil {
		logger.Debug("Failed to update approval message ID for banned word %d: %v", wordID, err)
		return err
	}
	logger.Debug("Successfully updated approval message ID for banned word %d", wordID)
	return nil
}
...
func (db *DB) GetBannedWordByApprovalMessageID(approvalMessageID string) (*BannedWord, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Looking up banned word by approval message ID: %s", approvalMessageID)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Debug("No banned word found for approval message ID: %s", approvalMessageID)
		} else {
			logger.Debug("Failed to get banned word by approval message ID %s: %v", approvalMessageID, err)
		}
		return nil, err
	}
	logger.Debug("Found banned word ID %d for approval message %s", bw.ID, approvalMessageID)
	return &bw, nil
}
...
func (db *DB) ApproveBannedWordByOpplysar(wordID int, approverID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Opplysar approving banned word ID %d by %s", wordID, approverID)
	...
	if err != nil {
		logger.Debug("Failed to approve banned word by opplysar ID %d: %v", wordID, err)
		return err
	}
	...
	if rows == 0 {
		logger.Debug("No pending banned word with ID %d found for opplysar approval", wordID)
		return fmt.Errorf("no pending banned word found for opplysar approval")
	}

	logger.Debug("Successfully approved banned word ID %d by opplysar", wordID)
	return nil
}
...
func (db *DB) ApproveBannedWordByRettskrivar(wordID int, approverID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Rettskrivar approving banned word ID %d by %s", wordID, approverID)
	...
	result, err := db.conn.Exec(query, approverID, wordID)
	if err != nil {
		logger.Debug("Failed to approve banned word by rettskrivar ID %d: %v", wordID, err)
		return err
	}
	...
	rows, _ := result.RowsAffected()
	if rows == 0 {
		logger.Debug("No opplysar approved banned word with ID %d found for rettskrivar approval", wordID)
		return fmt.Errorf("no opplysar approved banned word found for rettskrivar approval")
	}

	logger.Debug("Successfully approved banned word ID %d by rettskrivar", wordID)
	return nil
}
...
func (db *DB) ApproveBannedWordCombined(wordID int, opplysarApprovers, rettskrivarApprovers []string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Combined approval for banned word ID %d", wordID)
	...
	if err != nil {
		logger.Debug("Failed to approve banned word with combined approval ID %d: %v", wordID, err)
		return err
	}
	...
	rows, _ := result.RowsAffected()
	if rows == 0 {
		logger.Debug("No pending banned word with ID %d found for combined approval", wordID)
		return fmt.Errorf("no pending banned word found for combined approval")
	}

	logger.Debug("Successfully approved banned word ID %d with combined approval", wordID)
	return nil
}
...
func (db *DB) UpdateBannedWordForumThreadID(wordID int, forumThreadID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Updating forum thread ID for banned word %d: %s", wordID, forumThreadID)
	query := fmt.Sprintf("UPDATE %s SET forum_thread_id = ? WHERE id = ?", db.bannedWordsTable)
	_, err := db.conn.Exec(query, forumThreadID, wordID)
	if err != nil {
		logger.Debug("Failed to update forum thread ID for banned word %d: %v", wordID, err)
		return err
	}
	logger.Debug("Successfully updated forum thread ID for banned word %d", wordID)
	return nil
}
...
func (db *DB) RejectBannedWord(wordID int, rejectorID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Rejecting banned word ID %d by rejector %s", wordID, rejectorID)
	...
	if err != nil {
		logger.Debug("Failed to reject banned word ID %d: %v", wordID, err)
		return err
	}
	...
	rows, _ := result.RowsAffected()
	if rows == 0 {
		logger.Debug("No pending banned word with ID %d found for rejection", wordID)
		return fmt.Errorf("no pending banned word found for rejection")
	}

	logger.Debug("Successfully rejected banned word ID %d", wordID)
	return nil
}
...
func (db *DB) GetPendingBannedWord() (*BannedWord, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Retrieving next pending banned word")
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Debug("No pending banned words found")
			return nil, nil
		}
		logger.Debug("Failed to get pending banned word: %v", err)
		return nil, err
	}
	logger.Debug("Retrieved pending banned word ID %d: %s", bw.ID, bw.Word)
	return &bw, nil
}
...
func (db *DB) GetBannedWordByID(wordID int) (*BannedWord, error) {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Looking up banned word by ID: %d", wordID)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Debug("No banned word found with ID: %d", wordID)
		} else {
			logger.Debug("Failed to get banned word by ID %d: %v", wordID, err)
		}
		return nil, err
	}
	logger.Debug("Found banned word ID %d: %s", bw.ID, bw.Word)
	return &bw, nil
}
...
func (db *DB) AddBannedWordWithThread(word, reason, authorID, forumThreadID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Adding banned word: %s by %s with forum thread %s", word, authorID, forumThreadID)
	...
	if err != nil {
		logger.Debug("Failed to add banned word with thread: %v", err)
		return err
	}
	logger.Debug("Successfully added banned word: %s with forum thread %s", word, forumThreadID)
	return nil
}
...
func (db *DB) UpdateBannedWordThread(word, forumThreadID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Updating forum thread for banned word: %s to %s", word, forumThreadID)
	query := fmt.Sprintf("UPDATE %s SET forum_thread_id = ? WHERE word = ?", db.bannedWordsTable)
	_, err := db.conn.Exec(query, forumThreadID, word)
	if err != nil {
		logger.Debug("Failed to update forum thread for banned word %s: %v", word, err)
		return err
	}
	logger.Debug("Successfully updated forum thread for banned word: %s", word)
	return nil
}
...
func (db *DB) RemoveBannedWord(word string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Removing banned word: %s", word)
	query := fmt.Sprintf("DELETE FROM %s WHERE word = ?", db.bannedWordsTable)
	_, err := db.conn.Exec(query, word)
	if err != nil {
		logger.Debug("Failed to remove banned word: %v", err)
		return err
	}
	logger.Debug("Successfully removed banned word: %s", word)
	return nil
}
...
func (db *DB) AddStarboardMessage(originalMessageID, starboardMessageID, channelID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Adding starboard message mapping: %s -> %s", originalMessageID, starboardMessageID)
	...
	if err != nil {
		logger.Debug("Failed to add starboard message mapping: %v", err)
		return err
	}
	logger.Debug("Successfully added starboard message mapping")
	return nil
}
...
func (db *DB) UpdateStarboardMessage(originalMessageID, starboardMessageID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Updating starboard message mapping: %s -> %s", originalMessageID, starboardMessageID)
	...
	if err != nil {
		logger.Debug("Failed to update starboard message mapping: %v", err)
		return err
	}
	logger.Debug("Successfully updated starboard message mapping")
	return nil
}
...
func (db *DB) RemoveStarboardMessage(originalMessageID string) error {
	logger := logging.GetLogger("DATABASE")
	logger.Debug("Removing starboard message mapping for original message: %s", originalMessageID)
	query := fmt.Sprintf("DELETE FROM %s WHERE original_message_id = ?", db.starboardTable)
	_, err := db.conn.Exec(query, originalMessageID)
	if err != nil {
		logger.Debug("Failed to remove starboard message mapping: %v", err)
		return err
	}
	logger.Debug("Successfully removed starboard message mapping")
	return nil
}
...
func (db *DB) ClearDatabase() error {
	...
	if err != nil {
		logger.Debug("Failed to clear the database: %v", err)
		return err
	}
